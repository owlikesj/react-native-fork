...
function replaceModuleName(path, asset) {
  let pAarray = path.split('/')
  let moduleName = ''
  if (pAarray.length > 2) {
    moduleName = pAarray[pAarray.length - 2]
    path = path.replace(moduleName, asset.moduleName);
  }
...
class AssetSourceResolver {
  ...
  scaledAssetPathInBundle(): ResolvedAssetSource {
    let path = this.bundlePath || '';
    path = replaceModuleName(path, this.asset);
